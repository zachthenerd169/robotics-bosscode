...
void loop()
{
  if(Serial.available() > 0) 
  {
    ...
    {
      strafeRight(powerLevels[powerLevel]);
    }  
    
     // Strafe left
    else if(mode == 10)
    {
      strafeLeft(powerLevels[powerLevel]);
    }  
...
void strafeRight(int powerLevel)
{
    ST1.motor(1, -powerLevel);
    ST1.motor(2, powerLevel);
}

void strafeLeft(int powerLevel)
{
    ST1.motor(1, powerLevel);
    ST1.motor(2, -powerLevel/2);
}
